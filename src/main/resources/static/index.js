...
function getAllProvince() {
    $.ajax({
        ...
        url: `http://localhost:8080/province/get`,
        success: function (data) {
            let content = displayTable();
            for (let i = 0; i < data.length; i++) {
                content += "<tr>"
                content += "<th>"+ data[i].id +"</th>"
                content += "<th onclick='displayOneProduct("+ data[i].id +")'>"+ data[i].name +"</th>"
...
function getCategory() {
    $.ajax({
        type: "GET",
        //tên API
        url: `http://localhost:8080/province/country`,
...
function formDetail(data) {
    ...
    document.getElementById("popular1").value = data.popular
